...
public class UnifiedLoggingHandler implements HandlerInterceptor {
    ...
    @Around("execution(* com.smartdocs.question_service.controller..*(..)) || execution(* com.smartdocs.question_service.service..*(..))")
    public Object logExecution(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (className.contains("controller")) {
            layerLogger.info("Microservice: {} | [START] Request ID: {} Method: '{}' started", 
                    microserviceName, requestId, methodName);
        } else if (className.contains("service")) {
            String additionalInfo = extractServiceMethodContext(joinPoint);
            layerLogger.info("Microservice: {} | [START] Request ID: {} {}Fetching data or performing operation",
                    microserviceName, requestId,
        ...
        try {
            ...
            if (className.contains("controller")) {
                layerLogger.info("Microservice: {} | [END] Request ID: {} Method: '{}' completed in {} ms. Response: {}", 
                        microserviceName, requestId, methodName, elapsedTime,
                        result instanceof org.springframework.http.ResponseEntity
                                ? ((org.springframework.http.ResponseEntity<?>) result).getStatusCode()
                                : "Unknown");
            } else if (className.contains("service")) {
                String additionalInfo = extractServiceMethodContext(joinPoint);
                layerLogger.info("Microservice: {} | [END] Request ID: {} {} completed in {} ms", 
                        microserviceName, requestId,
                        additionalInfo != null ? additionalInfo : "", elapsedTime);
            }
        } catch (Exception ex) {
            layerLogger.error("Microservice: {} | [ERROR] Request ID: {} Method: '{}' failed: {}", 
                    microserviceName, requestId, methodName, ex.getMessage());
            throw ex;
        }
    ...
    private String extractServiceMethodContext(ProceedingJoinPoint joinPoint) {
        ...
        switch (methodName) {
            case "getAllQuestions":
                return "Fetching all questions.";
            case "getQuestionsByCategory":
                return args.length > 0 ? "Fetching questions for category: " + args[0] + "." : "";
            case "addQuestion":
                return args.length > 0 ? "Adding question: " + args[0] + "." : "";
            case "getQuestionsForQuiz":
                return args.length > 1 ? "Generating questions for category: " + args[0] + " with number of questions: " + args[1] + "." : "";
            case "getQuestionsFromId":
                return args.length > 0 ? "Fetching questions with IDs: " + args[0] + "." : "";
            case "getScore":
                return args.length > 0 ? "Calculating score for responses." : "";
            default:
                return null;
        }
    }
